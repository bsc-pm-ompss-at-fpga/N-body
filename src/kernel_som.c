...
#pragma omp task label(calculate_forces_BLOCK)
...
#pragma omp target device(fpga) \
  copy_inout([n_blocks*FORCE_FPGABLOCK_SIZE]forces) \
  copy_in([n_blocks*PARTICLES_FPGABLOCK_SIZE]particles)
#pragma omp task label(calculate_forces_task)
void calculate_forces_task(const int n_blocks, float * forces, const float * particles)
{
   #pragma HLS inline
...
#pragma omp target device(fpga) localmem_copies \
  copy_inout([PARTICLES_FPGABLOCK_SIZE]particles, [FORCE_FPGABLOCK_SIZE]forces)
#pragma omp task label(update_partices_BLOCK)
...
#pragma omp target device(fpga) \
  copy_inout([n_blocks*PARTICLES_FPGABLOCK_SIZE]particles, [n_blocks*FORCE_FPGABLOCK_SIZE]forces)
#pragma omp task label(update_particles_task)
void update_particles_task(const int n_blocks, float * particles,
      float * forces, const float time_interval)
{
   #pragma HLS inline
